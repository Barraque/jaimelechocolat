...
if(isset($_SESSION["id"]) and isset($_GET["quantity"]) and isset($_GET["idproduit"])){
    $stmt = $bdd->prepare("select count(id_panier) cmpt from panier where id_user =  ?");
    ...
    $stmt3 = $bdd->prepare("select count(*) cmpt from panier where id_produit = ? and id_user = ? ");
    ...
    if($res3["cmpt"] != 0){
        ...
        $stmt4 = $bdd->prepare("insert into panier (id_user, id_produit, qte, id_panier) values (?,?,?,?)");
